...
params = {
    ...
    'nc' : 1,# Number of channles in the training images. For greyscale images this is 1.
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        # Here, we will first train the discriminator with real images and then with fake images generated by the generator.
        # Real images are read from the labeled dataset, while fake images are generated by the generator using random noise as input.
        # After training the discriminator, we will train the generator to generate better fake images that can fool the discriminator.
        netD.zero_grad()
        ## Train with all-real batch
        # Transfer data tensor to GPU/CPU (device)
        real_img_data = data[0].to(device)
        # Get batch size. Can be different from params['nbsize'] for last batch in epoch.
        b_size = real_img_data.size(0)
        label = torch.full((b_size,), real_label, dtype=torch.float, device=device)
        # Forward pass real batch through D
        output = netD(real_img_data).view(-1)
